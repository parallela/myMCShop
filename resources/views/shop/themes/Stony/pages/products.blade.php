                     ...
                     @foreach($extras as $product)
                         ...
                         <div class="package">
                             ...
                             <div class="info">
                                 ...
                                 <div class="button">
                                     @if($product->max_buys == 1 && $userOrders->contains('product_id',$product->id))
                                         <button class="btn btn-sm btn-info btn-block toggle-modal disabled">@lang('messages.purchasedbtn')</button>
                                     @elseif($product->required_product_id != null && !$userOrders->contains('product_id',$product->required_product_id))
                                         <button data-toggle="modal" data-target="#p{{ $product->id }}"
                                                 class="btn btn-sm btn-info btn-block toggle-modal">@lang('messages.infobtn')</button>
                                     @else
                                         <button data-toggle="modal" data-target="#p{{ $product->id }}"
                                                 class="btn btn-sm btn-info btn-block toggle-modal">@lang('messages.buybtn')</button>
                                     @endif
                                 </div>
                             </div>
                         </div>
